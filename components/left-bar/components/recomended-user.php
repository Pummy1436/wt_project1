...
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['follow_recommended_user'])) {
...
<div class="bg-white drop-shadow-md rounded px-4 py-6">
    ...
    <div class="flex flex-col gap-2 mt-3">
        <?php
        $recommendedUserQuery = "SELECT * FROM users WHERE NOT id IN(SELECT receiver_id FROM user_follow WHERE sender_id = ?) AND id != ? LIMIT 3;";
        $stmt = $link->prepare($recommendedUserQuery);
        $stmt->bind_param("ii", $_SESSION["id"], $_SESSION["id"]);
        $stmt->execute();
        $all_recommended_user = $stmt->get_result();

        if (!($all_recommended_user->num_rows < 1)) {
            while ($row = $all_recommended_user->fetch_assoc()) { ?>
            ...
            <div class="pb-2 pt-8 text-sm text-center">Hmm. Looks like you're following everyone.</div>
        <?php } ?>
    </div>
</div>
