<div class="bg-white drop-shadow-md rounded p-5 my-3">
    <div class="flex items-center gap-2">
        <a href="profile.php?user_id=<?php echo $row["id"]; ?>">
            ...
            <?php } else { ?>
                <img src="../assets/images/user/profile/<?php echo $row["profile_picture"]; ?>" alt="Profile Picture" class="w-8 h-8 rounded-md object-cover" />
            <?php } ?>
        </a>
        ...
        <p class="text-gray-600 text-sm">@<?php echo $row["username"] ?> · <?php echo timeElapsedString($row["post_date"]) ?></p>
    </div>
    ...
    <a href="?category_id=<?php echo $row["category_id"]; ?>" class="text-indigo-600 text-base font-semibold">#<?php echo htmlspecialchars($row["category_title"]) ?></a>
    <?php if (isset($row["post_image"]) && $row["post_image"] != "") { ?>
        <a href="post.php?post_id=<?php echo $row["post_id"]; ?>">
            <img src="../assets/images/post/<?php echo htmlspecialchars($row["post_image"]); ?>" alt="Post Image" class="w-full h-72 object-cover shadow rounded mt-4" />
        </a>
    <?php } ?>
    <div class="flex mt-4">
        <form method="POST" class="flex items-center">
            <div class="flex items-center cursor-pointer">
                <button type="submit" name="post_like" class="focus:outline-none">
                    <?php
                ...
                <button type="submit" name="post_dislike" class="focus:outline-none">
        ...
        <div class="flex items-center ml-8 cursor-pointer hover:text-indigo-700">
            ...
            <?php
            $postCommentCountQuery = "SELECT COUNT(*) AS post_comment_count FROM post 
                                    INNER JOIN post_comment ON post_id = post_comment.post_comment_post_id 
                                    WHERE post_id = ? 
                                    GROUP BY post_comment_post_id";
            ...
            ?>
            <a href="post.php?post_id=<?php echo $row["post_id"]; ?>" class="text-sm px-1">
                <?php echo (!$result->num_rows) ? "0" : $post_comment_count["post_comment_count"]; ?> comments
            </a>
        </div>
    </div>
</div>
