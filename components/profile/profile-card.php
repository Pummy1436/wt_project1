<div class="rounded relative mt-2 h-48 mb-12">
    ...
    <?php } else { ?>
        <img src="../assets/images/user/banner/<?php echo $userData["banner_picture"]; ?>" alt="User Banner" class="w-full h-full object-cover absolute shadow rounded-t" />
    <?php } ?>
    <div class="w-20 h-20 rounded-full bg-cover bg-center bg-no-repeat absolute bottom-0 -mb-10 ml-12 shadow flex items-center justify-center">
        ...
        <?php } else { ?>
            <img src="../assets/images/user/profile/<?php echo $userData["profile_picture"]; ?>" alt="Profile Picture" class="absolute z-0 h-full w-full object-cover rounded-full shadow top-0 left-0 bottom-0 right-0" />
        <?php } ?>
    </div>
</div>
<div class="mx-12 flex items-center justify-between">
    <div>
        <div class="pb-2 text-base font-bold text-gray-800">
            <?php echo $userData["first_name"] . " " . $userData["last_name"]; ?> · 
            <span class="pb-2 text-sm font-normal text-gray-800">@<?php echo $userData["username"]; ?></span>
        </div>
        <div class="flex items-center text-sm gap-1">
            ...
            <span>Joined on <?php echo formatDateToEnglish($userData["user_date"]); ?></span>
        </div>
    </div>
    <div>
        <?php if (($user_id == null || $user_id == "undefined") || (!($user_id == null || $user_id == "undefined") && ($user_id != $user_profile_id))) {
