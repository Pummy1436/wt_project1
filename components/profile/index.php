...
if (empty($userData) || empty($user_profile_id)) { ?>
    <div class="pt-16 text-sm text-center">Sorry, the user you're looking for cannot be found.</div>
<?php } else {
    include("../components/profile/profile-card.php"); ?>
    <div class="border-b my-2"></div>
    <?php
    $userAllPostQuery = "SELECT * FROM post 
    ...
    if ($userAllPostData->num_rows > 0) {
        while ($row = $userAllPostData->fetch_assoc()) {
            include("../components/content/components/post-card.php");
        }
    }
} ?>
